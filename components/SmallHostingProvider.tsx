...
export function SmallHostingProvider() {
  ...
  return (
    <section className="group relative py-14 md:py-16 bg-[#030F16] border-b border-white/10 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Stats — reserve the layout space, fade in on hover */}
        <div className="flex flex-wrap justify-between items-baseline gap-y-10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-out">
          {stats.map((stat) => (
            <div
              key={stat.label}
      ...
      {/* Quote overlay — visible by default, fades out on hover */}
      <div className="absolute inset-0 flex items-center justify-center px-4 pointer-events-none opacity-100 group-hover:opacity-0 transition-opacity duration-500 ease-out">
        <h2 className="font-caveat text-4xl md:text-5xl lg:text-6xl font-bold text-green-50/50 tracking-tight leading-none text-center">
          A small hosting provider for friends
        </h2>
      </div>
    </section>
  );
